...
type Totals = Pick<TokenBreakdown, "input" | "output" | "cacheRead" | "reasoning">
  ...
  async parse(source, context) {
    ...
    for (const line of result.lines) {
      ...
      if (entry?.type === "turn_context" && payload) {
        ...
        model = textOf(modelInfo?.slug) ?? textOf(payload.model) ?? textOf(payload.model_name) ?? model
        provider = textOf(payload.model_provider) ?? provider
        continue
      }
      ...
      if (!tokens && total) tokens = previous ? subtract(total, previous) : total
      if (!tokens) continue
...
function readTotals(value: Record<string, unknown> | null): Totals | null {
  ...
  const cached = tokenCount(value.cached_input_tokens ?? value.cache_read_input_tokens)
  return {
