  ...
  return {
    ...
    async *discover(context) {
      ...
      for (const root of context.extraRoots) {
        for (const path of await walkFiles(join(root, options.id), options.file)) paths.add(path)
      }
      for (const path of paths) yield { agent: options.id, path, kind: options.kind ?? "jsonl" }
    },
    parse: options.parse,
  }
}
