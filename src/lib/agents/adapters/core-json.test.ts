...
describe("whole-file JSON adapters", () => {
      ...
      messages: [
        {
          ...
          model: "gemini-3",
          content: "secret",
          tokens: { input: 8, output: 4, cached: 2, thoughts: 3 },
        },
      ],
    })
    const source = await first(geminiCliAdapter, home, {
      GEMINI_CLI_HOME: geminiHome,
    })
    const event = (await geminiCliAdapter.parse(source, parseContext()))
      .events[0]
  ...
  test("Amp prefers ledger usage and reported credits", async () => {
    ...
    await json(path, {
      ...
      env: {
        initial: {
          ...
          trees: [
            { uri: "file:///other", repository: { url: "/work/other" } },
            { uri: "file:///tmp/worktree", repository: { url: "/work/app/." } },
          ],
        },
      },
      messages: [
        { role: "assistant", usage: { model: "wrong", inputTokens: 99 } },
      ],
      usageLedger: {
        events: [
          {
    ...
    expect(
      (await ampAdapter.parse(source, parseContext())).events[0]
    ).toMatchObject({
      project: "/work/app",
      tokens: {
        input: 11,
  ...
  test("Amp uses timestamped usage from actor snapshots", async () => {
    ...
    await json(path, {
      ...
      env: { initial: { workspaceRoot: "/work/app" } },
      messages: [
        {
          messageId: "M-one",
          role: "assistant",
  ...
  test("Droid maps settings totals and project sidecar directory", async () => {
    ...
    await json(path, {
      ...
      providerLockTimestamp: "2026-01-02T00:00:00Z",
      tokenUsage: {
        inputTokens: 10,
        outputTokens: 4,
        cacheReadTokens: 2,
    ...
    expect(
      (await droidAdapter.parse(source, parseContext())).events[0]
    ).toMatchObject({
      project: "my-project",
      tokens: {
        input: 10,
